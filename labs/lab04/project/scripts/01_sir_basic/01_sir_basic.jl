...
plot(
    agent_df.time, agent_df.susceptible, label="Восприимчивые",
...
plot!(agent_df.time, model_df.total, label="Всего (включая умерших)",
      linestyle=:dash)

savefig(plotsdir(script_name, "sir_basic_dynamics.png"))

@save datadir(script_name, "sir_basic_agent.jld2") agent_df
@save datadir(script_name, "sir_basic_model.jld2") model_df

println("✅ График и данные сохранены")
